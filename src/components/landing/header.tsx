...
export function Header()  {
  ...
  return (
      ...
      className={cn("fixed inset-x-0 top-0 z-20 mx-auto mt-4 flex w-[90%]  max-w-[80rem] justify-between rounded-2xl px-4 py-4 md:px-10",
      windowHeight > 20 && "bg-white/50 border border-border dark:bg-card/30 backdrop-blur-xl"
      )}
  ...
  >
    ...
    <div className="flex items-center gap-4">
      <nav className="hidden items-center justify-center gap-3 lg:flex">
       ...
       setActive={setActive} active={active} label="Resources">
       ...
       active={active} setActive={setActive} label="Blog">
        {Array.from({length:5}).map((_,i) => (
          <HeaderImageItem title="amazing content" key={i} description="Hello world, I am amzing javascript developer, javascript is the best language after croatian, thank u" />
        ))}
       </HeaderMenuSection>
       <HeaderMenuSection
       className="grid gap-2"
       active={active} setActive={setActive} label="Pricing">
          ...
          {active === label && (
            <div className="absolute left-1/2 top-[calc(100%_+_1rem)] -translate-x-2/3">
              ...
              >
                <motion.div
                  layout
                  className={cn("h-full w-max p-4", className)}
                >
                  {children}
                </motion.div>
              </motion.div>
